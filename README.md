...
- **OpenAI:** proof review is not currently wired into a runtime route. Production should add a server-only Responses API path with strict JSON schema, capped input, timeout/retry, and a configurable vision-capable model before enabling proof-based quests.
...
5. Validate demo navigation, Convex connectivity, and the no-credential fallback after deployment.
