...
export default function JamQuest() {
  ...
  if (!demo && page === "home") {
    ...
    return <Landing enter={() => setAuthScreen(true)} />;
  }
...
function Landing({ enter }: { enter: () => void }) {
  ...
  return <main className="landing outside-lands"><nav className="landing-nav"><b className="brand">jam<span>quest</span></b><span className="fan-made">FESTIVAL FIELD GUIDE · DEMO</span><button className="ghost" onClick={() => setOverviewOpen(true)}>Explore questbook</button></nav><section className="hero"><p className="kicker">OUTSIDE LANDS, YOUR WAY</p><h1>Find your<br /><em>foggy little world.</em></h1><p className="hero-copy">A colorful, fan-made festival companion for planning sets, building your crew, and collecting small moments—without tracking anyone’s exact location.</p><div className="hero-actions"><button className="primary" onClick={enter}>Enter the Lands <span>→</span></button></div><div className="festival-stamp"><span>CHAPTER 01</span><b>THE SIGNAL<br />STARTS HERE</b><i>Fog Coin field guide</i></div></section><section className="how"><p className="kicker">HOW THE FIELD GUIDE WORKS</p><h2>Set a plan. Leave room for wonder.</h2><div className="steps"><div><i>01</i><h3>Make your mark</h3><p>Build an avatar, save artists, and shape a crew that can find each other safely.</p></div><div><i>02</i><h3>Follow signals</h3><p>Complete small quests and pick up Fog Coins for virtual festival upgrades.</p></div><div><i>03</i><h3>Wander smarter</h3><p>Use optional routes and status check-ins—not continuous location tracking.</p></div></div></section>{overviewOpen && <div className="quest-preview-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setOverviewOpen(false); }}><section className="quest-preview" role="dialog" aria-modal="true" aria-labelledby="quest-preview-title"><button className="quest-preview-close" onClick={() => setOverviewOpen(false)} aria-label="Close quest adventure preview">×</button><p className="kicker">A PEEK INSIDE THE QUESTBOOK</p><h2 id="quest-preview-title">Your little adventure<br /><em>through the fog.</em></h2><p className="quest-preview-intro">Follow playful signals, make the festival yours, and watch your field guide grow. Here’s the journey—no quests can be started until you log in.</p><div className="quest-path" aria-label="Quest adventure overview"><article><i>✦</i><b>Make your signal</b><span>Create your identity and festival fit.</span></article><span className="quest-trail" aria-hidden="true">···</span><article><i>♫</i><b>Find your sound</b><span>Choose artists and shape your day.</span></article><span className="quest-trail" aria-hidden="true">···</span><article><i>⌁</i><b>Gather your crew</b><span>Coordinate without exact tracking.</span></article><span className="quest-trail" aria-hidden="true">···</span><article><i>⌖</i><b>Explore the map</b><span>Unlock routes, pins, and new corners.</span></article></div><div className="quest-preview-reward"><span>✹</span><div><b>Collect Fog Coins</b><p>Complete signals to unlock virtual field-guide features and festival flair.</p></div></div><div className="quest-preview-actions"><button className="primary" onClick={enter}>Log in to begin <span>→</span></button><button className="text-button" onClick={() => setOverviewOpen(false)}>Keep looking around</button></div></section></div>}</main>;
}
